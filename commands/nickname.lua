...
function command.run(message, mt)
  ...
  elseif mt[1] == "check" then
    ...
    if string.find(uj2f, uj.id, 10) then
      if numnames == 1 then
        message.channel:send(formatstring(lang.check_yourself_singular, {nicknamestring}))
      else
        message.channel:send(formatstring(lang.check_yourself_plural, {nicknamestring}))
      end
    else
      local numnames2 = 0
      for k in pairs(uj2.names) do numnames2 = numnames2 + 1 end
      if numnames2 == 1 then
        message.channel:send(formatstring(lang.check_other_singular, {nicknamestring, uj2.pronouns["their"]}))
      else
